...
# | scale\_pos\_weight=20 | \~0.84 | Improved |

# | lr=0.1, num\_leaves=15, no spw | \*\*0.8501\*\* | ✅ Best |

# 

# \## Final Recommendation

# Use: learning\_rate=0.1, num\_leaves=15, scale\_pos\_weight=None
...
Two Random Forest models were compared using identical 5-Fold Stratified Cross-Validation to measure the impact of adding simulated external contextual features (`ambient\_temp\_C`, `factory\_load\_pct`, `humidity\_pct`).
...
* Notes: External contextual signal enabled the model to better distinguish normal vs. failure conditions, without any row loss or data integrity issues during fusion (see `review\_notes.md`)
...
* SMOTE was applied inside CV folds only (no data leakage — verified in `progress\_week3.md`).
